...
class BreakContinue(Node):
    ...
    # "break" if this is a break statement, or "continue" if this is a continue statement
    descr = None
...
class DeclInfo:
    ...
    def __init__(self, identifier, ctype, span, storage=None, init=None, body=None, param_names=None):
    ...
    def process(self, il_code, symbol_table, c):
        ...
        var = symbol_table.add_variable(self.identifier, self.ctype, defined, linkage, storage)

        if self.init: self.do_init(var, storage, il_code, symbol_table, c)
    ...
    def do_body(self, il_code, symbol_table, c):
        ...
        for ctype, param, i in iterations:
            arg = symbol_table.add_variable(param, ctype, symbol_table.DEFINED, None, symbol_table.AUTOMATIC)
    ...
    def check_main_type(self):
        """Check if function signature matches signature expected of main. Raises an exception if this function
        ...
        if self.ctype.args:
            ...
            is_ptr_array = (second.is_pointer() and (second.arg.is_pointer() or second.arg.is_array()))
    ...
    def get_defined(self, symbol_table, c):
        ...
        elif self.ctype.is_function() and not self.body: return symbol_table.UNDEFINED
...
class Declaration(Node):
    ...
    def get_decl_infos(self, node):
        ...
        for decl, init in zip(node.decls, node.inits):
            with report_err():
                ...
                if ctype.is_function(): param_identifiers = self.extract_params(decl)
                else: param_identifiers = []

                out.append(DeclInfo(identifier, ctype, decl.r, storage, init, self.body, param_identifiers))
    ...
    def make_ctype(self, decl, prev_ctype):
        ...
        elif isinstance(decl, decl_nodes.Array): new_ctype = self.generate_array_ctype(decl, prev_ctype)
        elif isinstance(decl, decl_nodes.Function): new_ctype = self.generate_func_ctype(decl, prev_ctype)
        elif isinstance(decl, decl_nodes.Identifier): return prev_ctype, decl.identifier
    ...
    def make_specs_ctype(self, specs, any_dec):
        ...
        """
        spec_span = specs[0].r + specs[-1].r
        storage = self.get_storage([spec.kind for spec in specs], spec_span)
        const = token_kinds.const_kw in {spec.kind for spec in specs}

        struct_specs = {token_kinds.struct_kw}
        if any(s.kind in struct_specs for s in specs):
            node = [s for s in specs if s.kind in struct_specs][0]
            ...
            base_type = self.parse_struct_spec(node, redec)
        ...
        # is a typedef
            ...
            base_type = self.get_base_ctype(specs, spec_span)
    ...
    def get_base_ctype(specs, spec_span):
        ...
        our_base_specs = [str(spec.kind) for spec in specs if spec.kind in base_specs]
        ...
        raise CompilerError(descr, spec_span)
    ...
    def parse_struct_spec(self, node, redec):
        ...
        for member in node.members:
            ...
            for decl_info in decl_infos:
                with report_err():
                    self.check_struct_member_decl_info(
