...
class DeclInfo:
    ...
    def __init__(self, identifier, ctype, range, storage=None, init=None, body=None, param_names=None):
    ...
    def process(self, il_code, symbol_table, c):
        """Process given DeclInfo object. This includes error checking, adding the variable to the symbol table, and
        ...
        var = symbol_table.add(self.identifier, self.ctype, defined, linkage)

        storage = self.get_storage(defined, linkage, il_code)

        if storage == il_code.STATIC and self.init:
            raise NotImplementedError("initializer on static storage unsupported")

        name = self.identifier.content
        il_code.register_storage(var, storage, name)
        if linkage == symbol_table.EXTERNAL: il_code.register_extern_linkage(var, name)
        if defined: il_code.register_defined(var, name)

        if self.init: self.do_init(var, il_code, symbol_table, c)
        if self.body: self.do_body(il_code, symbol_table, c)
    ...
    def do_body(self, il_code, symbol_table, c):
        ...
        elif not il_code.always_returns(): il_code.add(control_cmds.Return(None))
    ...
    def get_linkage(self, symbol_table, c):
        ...
        if c.is_global and self.storage == DeclInfo.STATIC: linkage = symbol_table.INTERNAL
            ...
            if var and var.linkage: linkage = var.linkage
            else: linkage = symbol_table.EXTERNAL
        elif self.ctype.is_function() and not self.storage: linkage = symbol_table.EXTERNAL
        elif c.is_global and not self.storage: linkage = symbol_table.EXTERNAL
        else: linkage = None
    ...
    def get_defined(self):
        ...
        if self.storage == self.EXTERN and not (self.init or self.body): return False
        elif self.ctype.is_function() and not self.body: return False
        else: return True

    def get_storage(self, defined, linkage, il_code):
        """Determine the storage duration."""
        if not defined or not self.ctype.is_object(): storage = None
        elif linkage or self.storage == self.STATIC: storage = il_code.STATIC
        else: storage = il_code.AUTOMATIC
...
class Declaration(Node):
    ...
    def get_decl_infos(self, node, symbol_table):
        ...
        base_type, storage = self.make_specs_ctype(node.specs, any_dec, symbol_table)
        proc = zip(node.decls, node.ranges, node.inits)

        out = []
        for decl, range_, init in proc:
            with report_err():
                ctype, identifier = self.make_ctype(decl, base_type, symbol_table)

                if isinstance(decl, decl_nodes.Function):
                    param_names = [self.get_decl_infos(param, symbol_table)[0].identifier for param in decl.args]
                else: param_names = []

                out.append(DeclInfo(identifier, ctype, range_, storage, init, self.body, param_names))
    ...
    def make_ctype(self, decl, prev_ctype, symbol_table):
        ...
        """
        if isinstance(decl, decl_nodes.Pointer): new_ctype = PointerCType(prev_ctype, decl.const)
        elif isinstance(decl, decl_nodes.Array): new_ctype = ArrayCType(prev_ctype, decl.n)
            ...
            # Create a new scope because if we create a new struct type inside the function parameters, it should be
            # local to those parameters.
            symbol_table.new_scope()
            args = [self.get_decl_infos(decl, symbol_table)[0].ctype for decl in decl.args]
            symbol_table.end_scope()
            ...
            if not args and not self.body: new_ctype = FunctionCType([], prev_ctype, True)
            elif has_void: new_ctype = FunctionCType([], prev_ctype, False)
            else: new_ctype = FunctionCType(args, prev_ctype, False)
    ...
    def make_specs_ctype(self, specs, any_dec, symbol_table):
        ...
        any_dec - Whether these specifiers are used to declare a variable. This value is important because `struct A;`
        has a different meaning than `struct A *p;`, since the former forward-declares a new struct while the latter may
         reuse a struct A that already exists in scope.

        Return a `ctype, storage class` pair, where storage class is one of the above values.
        """
        spec_range = specs[0].r + specs[-1].r

        all_type_specs = (set(ctypes.simple_types) | {token_kinds.signed_kw, token_kinds.unsigned_kw,
                                                      token_kinds.struct_kw})

        type_specs = [str(spec.kind) for spec in specs if spec.kind in all_type_specs]
        ...
        if specs_str == "struct":
            ...
            # This is a redeclaration of a struct if there are no storage specifiers and it declares no variables.
    ...
    def get_base_ctype(self, specs_str, spec_range, const):
        ...
        specs = {
            ...
            "char unsigned": ctypes.unsign_char,

            "short": ctypes.short,
            "short signed": ctypes.short,
            "int short": ctypes.short,
            "int short signed": ctypes.short,
            "short unsigned": ctypes.unsign_short,
            "int short unsigned": ctypes.unsign_short,

            "int": ctypes.integer,
            "signed": ctypes.integer,
            "int signed": ctypes.integer,
            "unsigned": ctypes.unsign_int,
            "int unsigned": ctypes.unsign_int,

            "long": ctypes.longint,
            "long signed": ctypes.longint,
            "int long": ctypes.longint,
            "int long signed": ctypes.longint,
            "long unsigned": ctypes.unsign_longint,
            "int long unsigned": ctypes.unsign_longint,
        }
        ...
        descr = "unrecognized set of type specifiers"
        raise CompilerError(descr, spec_range)
    ...
    def get_storage(self, spec_kinds, spec_range):
        ...
        """
        storage_classes = {token_kinds.auto_kw: DeclInfo.AUTO, token_kinds.static_kw: DeclInfo.STATIC,
                           token_kinds.extern_kw: DeclInfo.EXTERN}

        storage = None
        for kind in spec_kinds:
            if kind in storage_classes and not storage: storage = storage_classes[kind]
            elif kind in storage_classes:
                descr = "too many storage classes in declaration specifiers"
                raise CompilerError(descr, spec_range)
