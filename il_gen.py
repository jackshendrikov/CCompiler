...
class ILValue:
    """Value that appears as an element in generated IL code.
        value_type (enum) - One of the values below describing the general type of value this is.
        ctype (Type) - C type of this value.
    """

    # Options for value_type:
    # Temporary value. These are used to store intermediate values in computations.
    TEMP = 1
...
class LiteralILValue(ILValue):
    ...
    # We want to literals to compare equal if they have the same value and type, so the ASM generation step can keep
    # track of their storage locations as one unit.
    def __eq__(self, other):
...
class SymbolTable:
    ...
    def lookup(self, name):
        """Look up the identifier with the given name. This function returns the ILValue object for the identifier,
    ...
    def lookup_tok(self, identifier):
        ...
        if ret:
            ...
            raise CompilerError(descr.format(identifier.content), identifier.file_name, identifier.line_num)
