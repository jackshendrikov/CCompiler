...
class Spot:
    ...
    REGISTER = 1
    # Stack spot. The `detail` attribute is an integer representing the offset from rbp, usually negative.
    STACK = 2
    # A literal value. This is a bit of a hack, since a literal value isn't /really/ a storage spot.
    # The detail attribute is a string representation of the value of this literal.
    LITERAL = 3
    ...
    def asm_str(self, size):
        ...
        """
        # This is kinda hacky, but since this function is so well-defined it's OK for now.
    ...
    def masm_str(self, size):
        ...
        if self.spot_type == self.REGISTER and size == 4:
            ...
            return "BYTE ptr[ebp-{}]".format(str(abs(self.detail)))
        elif self.spot_type == self.STACK and size == 4:
            return "DWORD ptr[ebp-{}]".format(str(abs(self.detail)))
        elif self.spot_type == self.LITERAL:
            return self.detail
        else:
            raise NotImplementedError("Unsupported spot_type/size combo")
...
EAX = Spot(Spot.REGISTER, "eax")
ESI = Spot(Spot.REGISTER, "esi")
EDX = Spot(Spot.REGISTER, "edx")
