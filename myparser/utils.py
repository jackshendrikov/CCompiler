"""Utilities for the myparser."""

from errors import CompilerError, Range
from contextlib import contextmanager
import copy

# In the myparser.py file, the main parse function sets this global variable to the list of tokens. Then, all functions
# in myparser can reference this variable rather than passing around the tokens list everywhere.
tokens = None


class SimpleSymbolTable:
    """Table to record every declared symbol. This is required to parse typedefs in C, because the parser must know
...
class ParserError(CompilerError):
    ...
    #
    # AT generates a message like "expected semicolon at '}'", GOT generates message like "expected semicolon, got '}'",
    # and AFTER generates a message like "expected semicolon after '15'" (if possible).
    #
    # As a very general guide, use AT when a token should be removed, use AFTER  when a token should be to be inserted
    # (esp. because of what came before), and GOT when a token should be changed.
    AT = 1
    GOT = 2
    AFTER = 3
    ...
    def __init__(self, message, index, tokens, message_type):
        ...
        if message_type == self.AT: super().__init__(f"{message} at '{tokens[index]}'", tokens[index].r)
        elif message_type == self.GOT: super().__init__(f"{message}, got '{tokens[index]}'", tokens[index].r)
        elif message_type == self.AFTER:
            if tokens[index - 1].r: new_range = Range(tokens[index - 1].r.end + 1)
            else: new_range = None
...
@contextmanager
def log_error():
    """Wrap this context manager around conditional parsing code. For example,
        with log_error():
...
def match_token(index, kind, message_type, message=None):
    ...
    """
    global tokens
    if not message: message = f"expected '{kind.text_repr}'"

    if token_is(index, kind): return index + 1
    else: raise ParserError(message, index, tokens, message_type)
...
def add_range(parse_func):
    """Return a decorated function that tags the produced node with a range. Accepts a parse_* function, and returns a
