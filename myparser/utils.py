...
# This is a little bit messy, but worth the repetition it saves. In the myparser.py file, the main parse function sets
# this global variable to the list of tokens. Then, all functions in myparser can reference this variable rather than
# passing around the tokens list everywhere.
tokens = None


class ParserError(CompilerError):
    """Class representing myparser errors.
    ...
    def __init__(self, message, index, tokens, message_type):
        ...
        if message_type == self.AT:
            super().__init__(f"{message} at '{tokens[index]}'", tokens[index].r)
        elif message_type == self.GOT:
            super().__init__(f"{message}, got '{tokens[index]}'", tokens[index].r)
...
def log_error(error):
    """Log the error in the myparser to be used for error reporting. The value of error.amount_parsed is used to determine
    the amount successfully parsed before encountering the error.
...
def match_token(index, kind, message_type, message=None):
    """Raise ParserError if tokens[index] is not of the expected kind. If tokens[index] is of the expected kind, returns
    index + 1. Otherwise, raises a ParserError with the given message and message_type.
    """
    global tokens
    if not message:
